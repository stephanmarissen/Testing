...
mysql -uroot -ppassword < dbinit.sql

if [ -n "$SELECTDB" ]; then
	if [ -z "$MYSQLIMPORT" ]; then
		echo "[WARNING]: You have not provided a table to import!"
	else
		mysql -uroot -ppassword -e "USE $SELECTDB;"
		mysql -uroot -ppassword $SELECTDB < sqlfiles/$MYSQLIMPORT
	fi
fi
...
echo -n "[Entrypoint] Container IP Address is: "; cat /etc/hosts | tail -1
